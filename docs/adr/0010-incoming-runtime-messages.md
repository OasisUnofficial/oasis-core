...
This proposal updates the runtime executor parameters (stored under the
`executor` field of the runtime descriptor) as follows:
...
- The `executor.max_in_messages` field in the runtime descriptor is checked. If
  it is equal to zero the method fails with `ErrIncomingMessageQueueFull`.
...
- If the value of the `size` field in the metadata structure is equal to or
  larger than the value of the `executor.max_in_messages` field in the runtime
  descriptor the method fails with `ErrIncomingMessageQueueFull`.
...
type Backend interface {
    ...
    // runtime.
    IncomingMessages(ctx context.Context, query *RuntimeQuery) ([]*IncomingMessage, error)
}
```
...
are queried at the start of each round and are then passed to the runtime via
...
- After a successful round, the `InMessagesCount` field of the compute body is
  ...
  accoordingly by decrementing the value of the `size` field.
