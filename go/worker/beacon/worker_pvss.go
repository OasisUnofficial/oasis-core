...
var pvssStateStoreKey = []byte("pvss round state")

type pvssRoundState struct {
...
func (w *pvssWorker) Quit() <-chan struct{} {
	return w.quitCh
}

func (w *pvssWorker) Cleanup() {
}

func (w *pvssWorker) worker() {
	defer func() {
		close(w.quitCh)
	}()
	...
	eventCh, eventSub, err := w.backend.WatchLatestPVSSEvent(w.parent.ctx)
	if err != nil {
	...
	for {
		...
		pvssState, err := w.backend.GetPVSSState(w.parent.ctx, ev.Height)
		if err != nil {
...
func (w *pvssWorker) eventOkForState(ev *beacon.PVSSEvent, expectedState beacon.RoundState) bool {
	if w.state == nil {
		return false
	}
...
func (w *pvssWorker) onStateCommit(ev *beacon.PVSSEvent, pvssState *beacon.PVSSState) {
	if w.state != nil {
		if ev.Epoch == w.state.Epoch && ev.Round == w.state.Round {
	...
	for _, id := range ev.Participants {
		if id.Equal(w.parent.identity.NodeSigner.Public()) {
			isPresent = true
		}
	...
	instance, err := pvss.New(&pvss.Config{
		PrivateKey:   &w.parent.identity.BeaconScalar,
		Participants: points,
...
func (w *pvssWorker) onStateReveal(ev *beacon.PVSSEvent, pvssState *beacon.PVSSState) {
	var stateOk bool
	defer func() {
		if !stateOk {
			w.persistState(nil)
	...
	if pvssState.BadParticipants[w.parent.identity.NodeSigner.Public()] {
...
func (w *pvssWorker) onStateComplete(ev *beacon.PVSSEvent, pvssState *beacon.PVSSState) {
...
func (w *pvssWorker) recoverPersistedState() {
	var state pvssRoundState
	if err := w.store.GetCBOR(pvssStateStoreKey, &state); err != nil {
	...
	pvssState, err := w.backend.GetPVSSState(w.parent.ctx, consensus.HeightLatest)
	if err != nil {
	...
	if err = state.Instance.SetScalar(&w.parent.identity.BeaconScalar); err != nil {
...
func (w *pvssWorker) persistState(newState *pvssRoundState) {
	w.state = newState

	if newState == nil {
		if err := w.store.Delete(pvssStateStoreKey); err != nil {
	...
	if err := w.store.PutCBOR(pvssStateStoreKey, newState); err != nil {
...
func (w *pvssWorker) retrySubmitTx(tx *transaction.Transaction) {
...
func newPVSS(
	...
	if parent.identity.BeaconScalar.Inner() == nil {
	...
	pvssBackend, shouldEnable := parent.consensus.Beacon().(beacon.PVSSBackend)

	w := &pvssWorker{
		parent:   parent,
		logger:   logging.GetLogger(workerName + "/pvss"),
		backend:  pvssBackend,
		registry: parent.consensus.Registry(),
		store:    serviceStore,
		state:    nil,
