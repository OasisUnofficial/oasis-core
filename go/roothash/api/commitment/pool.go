...
func (p *Pool) addOpenExecutorCommitment( // nolint: gocyclo
	...
	if err := sv.VerifyTxnSchedulerSigner(body.ProposalSignature, blk.Header.Round); err != nil {
	...
	ok, err := body.VerifyProposalSignature(p.Runtime.ID, blk.Header)
	if err != nil {
		logger.Error("failed to verify proposal signature",
