...
func (s *runtimeState) generateExecutorCommitments(t *testing.T, consensus consensusAPI.Backend, identity *identity.Identity, child *block.Block) (
	...
	for _, node := range executorNodes {
		...
		proposalHeader := &commitment.ProposalHeader{
			BatchHash:      commitBody.BatchHash,
			PreviousHeader: child.Header,
		}
		...
		var signedProposalHeader *commitment.SignedProposalHeader
		signedProposalHeader, err = proposalHeader.Sign(schedulerNode.Signer)
		require.NoError(err, "ProposalHeader.Sign")
		commitBody.ProposalSignature = signedProposalHeader.Signature
...
func testSubmitEquivocationEvidence(t *testing.T, backend api.Backend, consensus consensusAPI.Backend, identity *identity.Identity, states []*runtimeState) {
	...
	signedBatch1, err := batch1.Sign(node.Signer)
	require.NoError(err, "ProposalHeader.Sign")
	...
	signedBatch2, err := batch2.Sign(node.Signer)
	require.NoError(err, "ProposalHeader.Sign")
